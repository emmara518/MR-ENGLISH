...
export const FloatingFeatureCard: React.FC<FloatingFeatureCardProps> = ({
  ...
  return (
    <div
      ...
      className={`bg-white/95 backdrop-blur-md px-3.5 py-2.5 rounded-2xl shadow-lg border border-[#E9EEF0] flex items-center space-x-3 transition-all hover:translate-y-[-2px] hover:shadow-xl ${className}`}
    >
...
export const FloatingQuoteCard: React.FC<{ className?: string }> = ({ className = '' }) => {
  return (
    <div
      ...
      className={`bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-xl border border-[#E9EEF0] max-w-[190px] relative transition-transform hover:scale-105 ${className}`}
    >
      <span className="text-2xl leading-none text-slate-300 font-serif absolute -top-1 left-3">“</span>
