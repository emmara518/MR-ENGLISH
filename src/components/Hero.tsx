...
import { floatingFeatures, heroStats, instructorData, INSTRUCTOR_FALLBACK_IMAGE } from '../data/mockData';
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-10 lg:gap-14 xl:gap-20 items-center">
        ...
        <div className="z-10 flex flex-col justify-center w-full max-w-2xl mx-auto lg:mx-0 text-right">
          {/* Eyebrow */}
          <div className="flex items-center gap-2.5 mb-3 sm:mb-5" id="hero-eyebrow">
            <span className="w-7 sm:w-8 h-1.5 bg-[#C6D94E] rounded-full inline-block" />
            <span className="text-[11px] sm:text-xs md:text-sm font-extrabold uppercase tracking-[0.15em] text-[#16232E]">
              أكتر من مجرد إنجليزي
            </span>
          </div>

          {/* Large Editorial Headline */}
          <h1 className="text-4xl xs:text-5xl sm:text-6xl md:text-7xl lg:text-[76px] xl:text-[88px] font-black text-[#16232E] tracking-tight leading-[1.15] sm:leading-[1.1] mb-5 sm:mb-6 select-none" id="hero-title">
            <span className="block">ابنِ</span>
            <span className="relative inline-block my-0.5 sm:my-1">
              مستقبلاً ألمع
              {/* Lime accent highlighter stroke */}
              <span className="absolute right-0 bottom-1 sm:bottom-2 w-full h-2.5 sm:h-5 bg-[#C6D94E]/85 -z-10 rounded-sm" />
            </span>
            <span className="block">بنفسك</span>
          </h1>

          {/* Supporting Paragraph */}
          <p className="text-sm sm:text-base md:text-lg text-[#52616B] font-medium leading-relaxed max-w-xl mb-6 sm:mb-8 lg:mb-10">
            إنجليزي عملي. مهارات حقيقية. مستقبل ألمع. انضم لمنصة MR-ENGLISH واخطُ خطوتك الجاية مع <span className="font-bold text-[#16232E]">مستر محمد عبد الغفار</span>.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-5 mb-8 sm:mb-10 lg:mb-14">
            <button
              ...
              className="inline-flex items-center gap-2.5 px-6 sm:px-8 py-3 sm:py-3.5 rounded-full text-sm sm:text-base font-bold text-[#16232E] bg-[#C6D94E] hover:bg-[#B8CC42] shadow-sm hover:shadow-md transition-all transform active:scale-95 cursor-pointer"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2.5 sm:gap-3 px-4 sm:px-5 py-2.5 sm:py-3 rounded-full text-sm sm:text-base font-bold text-[#16232E] hover:text-[#16232E] group transition-colors cursor-pointer"
            >
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-[#16232E] flex items-center justify-center text-white transition-transform group-hover:scale-105">
                <Play className="w-3 sm:w-3.5 h-3 sm:h-3.5 fill-white -ml-0.5 rotate-180" />
              </div>
        ...
        <div className="relative flex items-center justify-center lg:justify-end w-full mt-4 sm:mt-0">
          ...
          <div className="relative w-full max-w-[420px] sm:max-w-[500px] md:max-w-[540px] xl:max-w-[580px] h-[480px] sm:h-[540px] lg:h-[620px] flex items-end justify-center mx-auto lg:ml-auto lg:mr-0">
            ...
            <div className="absolute right-1 sm:right-4 top-8 sm:top-14 z-20 pointer-events-none select-none rotate-6 text-right">
            ...
            <div className="relative w-full h-full flex items-end justify-center overflow-visible z-10">
              <img
                ...
                onError={(e) => {
                  ...
                  if (target.src !== INSTRUCTOR_FALLBACK_IMAGE) {
                    target.src = INSTRUCTOR_FALLBACK_IMAGE;
                  }
                }}
                alt="مستر محمد عبد الغفار - مدرس اللغة الإنجليزية"
                className="w-[86%] sm:w-[80%] h-[90%] sm:h-[94%] object-cover object-top filter contrast-[1.03] saturate-[1.05] rounded-b-[24px]"
                style={{
            ...
            <div className="absolute left-0 sm:-left-2 top-4 sm:top-8 lg:top-10 z-30 flex flex-col gap-2 sm:gap-2.5 scale-90 sm:scale-100 origin-top-left">
            ...
            {/* Floating Quote Note */}
            <div className="absolute left-0 sm:left-2 bottom-20 sm:bottom-28 z-30 hidden sm:block">
              <FloatingQuoteCard />
            </div>

            {/* Dark Floating Instructor Badge across the bottom */}
            <div className="absolute right-2 sm:right-8 md:right-10 left-2 sm:left-8 md:left-10 bottom-3 sm:bottom-6 z-30">
              <InstructorCard
