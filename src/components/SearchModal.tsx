...
export const SearchModal: React.FC<SearchModalProps> = ({
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl border border-[#E5EAE8]">
        {/* Search Input Bar */}
        <div className="p-4 border-b border-[#E5EAE8] flex items-center space-x-3">
          <Search className="w-5 h-5 text-[#52616B] shrink-0" />
          <input
            type="text"
            placeholder="Search courses, grammar, IELTS, pronunciation..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            autoFocus
            className="w-full bg-transparent text-sm sm:text-base font-medium text-[#16232E] focus:outline-none placeholder:text-slate-400"
          />
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-2.5 bg-[#F5F6F4] border-b border-[#E5EAE8] flex items-center space-x-2 overflow-x-auto text-xs">
          <span className="text-slate-400 font-medium">Quick:</span>
          {['Beginners', 'Grammar', 'IELTS', 'Conversation'].map((item) => (
            <button
              key={item}
            ...
            results.map((course) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="text-[10px] font-bold uppercase text-[#52616B]">
                      {course.category}
                    </span>
                    <h4 className="text-xs sm:text-sm font-bold text-[#16232E] group-hover:text-[#16232E]">
                      {course.title}
                    </h4>
                    <div className="flex items-center space-x-1.5 text-[11px] text-[#52616B] mt-0.5">
                ...
                <div className="w-7 h-7 rounded-full bg-[#E9EEF0] group-hover:bg-[#C6D94E] flex items-center justify-center transition-colors">
                  <ArrowRight className="w-3.5 h-3.5 text-[#16232E]" />
                </div>
              </div>
            ))
          ) : (
