...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div
      ...
      id="auth-modal"
    >
      <div className="relative w-full max-w-md bg-white rounded-3xl p-6 sm:p-8 shadow-2xl border border-[#E5EAE8]">
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-8 h-8 rounded-full bg-[#F5F6F4] hover:bg-[#E9EEF0] flex items-center justify-center text-slate-500"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="text-center mb-6">
          ...
          <h3 className="text-xl font-black text-[#16232E] mt-3">
            {mode === 'register' ? 'Begin Your English Journey' : 'Welcome Back'}
          </h3>
          <p className="text-xs text-[#52616B] mt-1">
            {mode === 'register'
              ? 'Join Mr. Mohamed Abdelghaffar to build a brighter future.'
        ...
        {submitted ? (
          <div className="py-10 text-center space-y-3">
            ...
            <h4 className="text-base font-bold text-[#16232E]">
              {mode === 'register' ? 'Welcome to MR-ENGLISH!' : 'Signed in successfully!'}
            </h4>
            <p className="text-xs text-[#52616B]">Redirecting to student dashboard...</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'register' && (
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-[#F5F6F4] border border-[#E5EAE8] text-sm text-[#16232E] focus:outline-none focus:border-[#16232E]"
                />
              </div>
            )}
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-full text-xs sm:text-sm font-bold text-[#16232E] bg-[#C6D94E] hover:bg-[#B8CC42] flex items-center justify-center space-x-2 shadow-xs transition-transform active:scale-98 cursor-pointer mt-2"
            >
              <span>{mode === 'register' ? 'Create Account & Start' : 'Sign In'}</span>
